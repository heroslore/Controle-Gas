...
Espaço separado do Controle de Gás: tudo do CRM fica dentro desta pasta `crm/`.

**Endereço (depois de ir para o `main`):** https://heroslore.github.io/Controle-Gas/crm/
...
```
Meta Ads ──(todo dia 08:20, GitHub Actions)──> crm/dados/meta.json ──> site republicado
```

O workflow `.github/workflows/crm-coleta.yml` roda `crm/coletor/coletar.py`, que:
...
- calcula os alertas de ontem (gastou e não gerou mensagem, custo por mensagem alto,
  frequência alta);
- grava tudo em `crm/dados/meta.json`, mantendo o histórico antigo e refazendo só os
...
```bash
python3 crm/coletor/coletar.py --verificar        # testa credenciais e acesso
python3 crm/coletor/coletar.py --dry-run          # coleta e mostra o resumo sem gravar
python3 crm/coletor/coletar.py                    # atualiza crm/dados/meta.json (últimos 45 dias + histórico)
python3 crm/coletor/coletar.py --completo         # baixa o histórico inteiro de novo
python3 -m unittest crm/coletor/test_coletar.py   # testes do coletor
python3 -m http.server 8000 --directory crm       # abre o painel em http://localhost:8000
```
