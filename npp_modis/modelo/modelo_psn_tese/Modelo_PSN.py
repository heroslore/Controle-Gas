...
Cada bioma tem sua própria subpasta dentro de "saidas_figuras/":
    ...
    saidas_figuras/
    ├── MA/
    │   ├── coeficientes_ridge_MA.csv
    │   ├── analise_residuos_MA.png
    │   ├── obs_vs_pred_MA.png
    │   ├── yrandomization_MA.png        (se RODAR_YRANDOMIZATION=True)
    │   ├── selecao_grau_polinomial_MA.png (se TESTAR_GRAUS=True)
    │   └── ...
    ├── CE/
    │   └── ...
    ├── CA/
    │   └── ...
    └── resumo_geral.csv   (só quando roda 'TODOS' — tabela comparativa)

Cada nova execução SOBRESCREVE os arquivos da rodada anterior (mesmo nome
sempre) — não fica acumulando "_1", "_2", "_3"... a cada rodada.
...
# na mesma pasta do script. Cada nova execução SOBRESCREVE os arquivos da
# rodada anterior (mesmo nome sempre, sem numeração acumulada).
# =============================================================================
...
def proximo_nome(pasta, nome_base, extensao):
    """
    Nome fixo 'nome_base.extensao' dentro da pasta do bioma — cada nova
    execução SOBRESCREVE o arquivo da rodada anterior (sem numeração).
    """
...
def classificar_fase_enso_oficial(df, min_estacoes=5):
    """
    Classifica cada mês em La Niña / Neutro / El Niño pela definição
    OPERACIONAL da NOAA/CPC: o ONI precisa ficar >= +0.5 (El Niño) ou
    <= -0.5 (La Niña) por pelo menos `min_estacoes` estações consecutivas e
    sobrepostas (padrão NOAA = 5). Sequências mais curtas ficam como Neutro,
    mesmo cruzando o limiar. Esta coluna não é usada pelo modelo Ridge em si
    (nenhum bioma usa 'Enso' como preditor) — é gerada aqui apenas para
    manter o arquivo Dados_base_nova_2001_2025.xlsx consistente com o que o
    Analise_Biomas_PSN.py espera, já que os dois scripts compartilham esse
    mesmo arquivo.
    """
    tmp = df[['ANO', 'MÊS', 'ONI']].dropna(subset=['ONI']).copy()
    tmp = tmp.sort_values(['ANO', 'MÊS']).reset_index(drop=True)

    ano = tmp['ANO'].astype(int).values
    mes = tmp['MÊS'].astype(int).values
    oni = tmp['ONI'].values
    n   = len(tmp)
    fase = np.full(n, 'Neutro', dtype=object)

    def proximo_mes(a, m):
        return (a, m + 1) if m < 12 else (a + 1, 1)

    def blocos_consecutivos(mask):
        blocos = []
        i = 0
        while i < n:
            if not mask[i]:
            ...
            while (j + 1 < n and mask[j + 1]
                   and proximo_mes(ano[j], mes[j]) == (ano[j + 1], mes[j + 1])):
                j += 1
            blocos.append((i, j))
            i = j + 1
        return blocos

    for ini, fim in blocos_consecutivos(oni >= 0.5):
        if (fim - ini + 1) >= min_estacoes:
            fase[ini:fim + 1] = 'El Niño'
    for ini, fim in blocos_consecutivos(oni <= -0.5):
        if (fim - ini + 1) >= min_estacoes:
            fase[ini:fim + 1] = 'La Niña'

    tmp['Enso'] = fase
    return tmp[['ANO', 'MÊS', 'Enso']]
...
def preparar_base_a_partir_do_csv():
    ...
    # Classifica a fase ENSO (não usada pelo modelo, só para manter o
    # arquivo compartilhado consistente com o Analise_Biomas_PSN.py) usando
    # a série de ONI completa, antes de remover linhas por precipitação
    # ausente (o ONI existe mesmo nesses meses).
    fases_enso = classificar_fase_enso_oficial(df)
    df = df.merge(fases_enso, on=['ANO', 'MÊS'], how='left')
...
GRAU_MODELO       = 3     # altere aqui o grau do modelo principal
TESTAR_GRAUS      = True  # True para comparar graus | False para pular
GRAU_MAXIMO_TESTE = 5     # até qual grau comparar (válido se TESTAR_GRAUS=True)

# Grade de alpha ampliada em SUPERSET estrito da grade original
# ([0.1, 1.0, 10.0, 50.0, 100.0]): mantém todos os valores antigos e só
# acrescenta valores mais altos, necessários para regularizar adequadamente
# os 55 termos do grau 3 (a grade antiga foi calibrada pensando nos 20 termos
# do grau 2). Por ser superset, rodar grau 2 com esta grade reproduz
# exatamente o mesmo alpha escolhido e os mesmos resultados de antes.
ALPHA_GRID = [0.1, 1.0, 10.0, 50.0, 100.0, 250.0, 500.0, 1000.0,
              2500.0, 5000.0, 10000.0, 50000.0, 100000.0]

RODAR_YRANDOMIZATION = False   # True para rodar | False para pular
...
BIOMA_ATIVO = os.environ.get('BIOMA_ATIVO', 'CE')  # altere aqui: 'MA', 'CE', 'CA' ou 'TODOS'
...
def rodar_modelo_bioma(bioma, dados_total):
    ...
    def _fit_eval_fold(x_tr, x_te, y_tr, y_te):
        ...
        gs = GridSearchCV(Ridge(), param_grid={'alpha': ALPHA_GRID},
                          cv=5, scoring='r2')
    ...
    if RODAR_YRANDOMIZATION:
        ...
        print("Bloco de Y-randomization pulado. Defina RODAR_YRANDOMIZATION = True para rodar.")
